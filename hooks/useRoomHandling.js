  ...
  const setupSocket = useCallback(async () => {
    try {
      ...
      if (socketRef.current) {
        ...
        // userRooms는 userId를 키로 사용해야 함
        if (userRooms?.get(user.id)) {
          const roomIdToLeave = userRooms.get(user.id);
          if (roomIdToLeave) {
            await new Promise((resolve) => {
              currentSocket.emit("leaveRoom", roomIdToLeave);
  ...
  const loadInitialMessages = useCallback(
    async (roomId) => {
      ...
      try {
        ...
        if (!socketRef.current?.connected) {
          Toast.warn("소켓 연결이 끊어져 재연결을 시도합니다.");
          await setupRoom(); // 전체 연결 및 방 입장까지 재시도
  ...
  const setupRoom = useCallback(async () => {
    ...
    setupPromiseRef.current = (async () => {
      try {
        ...
        // 1. Socket Setup
        socketRef.current = await setupSocket().catch((error) => {
